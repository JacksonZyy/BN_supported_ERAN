...
def handle_batchnormalization_layer(man, element, weight_para, bias_para, output_size, input_size, parameter_length, predecessors, num_predecessors):
    """
    ...
    element : ElinaAbstract0Ptr
        Pointer to the ElinaAbstract0 abstract element.
    weight_para: POINTER(double)
        real weights for each neuron
    bias_para: POINTER(double)
        real biases for each neuron
    input_size: POINTER(c_size_t)
        size of the input
    parameter_length: c_size_t
        number of parameters in BN
    predecessors: POINTER(c_size_t)
        the layers before the current layer
    num_predecessors: c_size_t
        the number of predecessors of the current layer
    ...
    """
    try:
        handle_batchnormalization_layer_c = fppoly_api.handle_batchnormalization_layer
        handle_batchnormalization_layer_c.restype = None
        handle_batchnormalization_layer_c.argtypes = [ElinaManagerPtr, ElinaAbstract0Ptr, ndpointer(ctypes.c_double),
                                                 ...
                                                 ndpointer(ctypes.c_size_t), c_size_t, POINTER(c_size_t), c_size_t]
        handle_batchnormalization_layer_c(man, element, weight_para, bias_para, output_size, input_size, parameter_length, predecessors, num_predecessors)
    except Exception as inst:
