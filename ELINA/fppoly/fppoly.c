...
void handle_convolutional_layer(elina_manager_t* man, elina_abstract0_t* element, double *filter_weights, double * filter_bias, size_t * input_size, size_t *filter_size, size_t num_filters, size_t *strides, size_t *output_size, size_t pad_top, size_t pad_left, bool has_bias, size_t *predecessors, size_t num_predecessors){
...
void handle_batchnormalization_layer(elina_manager_t* man, elina_abstract0_t* element, double *weight_para, double * bias_para, size_t *output_size, size_t *input_size, size_t parameter_length, size_t *predecessors, size_t num_predecessors){
	//printf("conv intermediate starts here\n");
	//fflush(stdout);
	assert(num_predecessors==1);
	fppoly_t *fp = fppoly_of_abstract0(element);
	...
	for(out_x=0; out_x < output_size[0]; out_x++) {
	    for(out_y = 0; out_y < output_size[1]; out_y++) {
		 for(out_z=0; out_z < output_size[2]; out_z++) {
		     ...
		     size_t *dim = (size_t *)malloc(sizeof(double));
		     size_t filter_index = out_z;
		     coeff[0] = weight_para[filter_index];
		     dim[0] = mat_x;
		     actual_coeff++;
		     double cst = bias_para[filter_index];
