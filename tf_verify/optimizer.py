    ...
    def get_abstract_element(self, nn, i, execute_list, output_info, domain):
        ...
        while i < nbr_op:
            if self.operations[i] == "MatMul":
                ...
                i += 1
            elif self.operations[i] == "BatchNormalization":
                scale_para, bias_para, mean_para, var_para, epsilon, image_shape, c_input_names, output_name, b_output_shape = self.resources[i][domain]
                nn.layertypes.append('BatchNormalization')
                nn.numlayer+=1
                if domain == 'deepzono':
                    assert "Implementation of BN layers for deepzono domain hasn't been implemented yet!"
                else:
                    execute_list.append(DeeppolyBNNode(scale_para, bias_para, mean_para, var_para, epsilon, image_shape, c_input_names, output_name, b_output_shape))
                i += 1
