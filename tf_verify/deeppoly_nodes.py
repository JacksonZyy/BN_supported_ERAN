...
class DeeppolyBNNode:
    def __init__(self, scale_para, bias_para, mean_para, var_para, epsilon, image_shape, input_names, output_name, output_shape):
        """
        collects the information needed for the bn_handle_layer transformer and brings it into the required shape
        ...
        image_shape : numpy.ndarray
            1D array of ints with 3 entries [height, width, channels] representing the shape of the of the image that is passed to the conv-layer
        """
        self.scale_para = np.ascontiguousarray(scale_para, dtype=np.double)
        self.bias_para = np.ascontiguousarray(bias_para, dtype=np.double)
        self.mean_para = np.ascontiguousarray(mean_para, dtype=np.double)
        self.var_para = np.ascontiguousarray(var_para, dtype=np.double)
        self.image_shape = np.ascontiguousarray(image_shape, dtype=np.uintp)
        self.out_size = (c_size_t * 3)(output_shape[1], output_shape[2], output_shape[3])
    ...
    def get_arguments(self):
        ...
        facilitates putting together all the arguments for the transformers in the child classes
        """
        parameter_length = self.scale_para.shape[0]
        # Compute the real bias and weight parameter in here and directly pass to ELINA
        rweight_para = []
        rbias_para = []
        for i in range(parameter_length):
            #rweight_para.append(1.0)
            #rbias_para.append(0.0)
            rwight =self.scale_para[i]/math.sqrt(self.var_para[i]+self.epsilon)
            rweight_para.append(rwight)
            rbias = self.bias_para[i] - (rwight * self.mean_para[i])
            rbias_para.append(rbias)
            
        return np.ascontiguousarray(rweight_para, dtype=np.double), np.ascontiguousarray(rbias_para, dtype=np.double), self.out_size, self.image_shape, parameter_length, self.predecessors, len(
            self.predecessors)

    def transformer(self, nn, man, element, nlb, nub, relu_groups, refine, timeout_lp, timeout_milp,
                    use_default_heuristic, testing):
        """
        transformer for a batchnormalization layer, if that layer is an intermediate of the network
        ...
        output : ElinaAbstract0Ptr
            abstract element after the transformer
        """
        handle_batchnormalization_layer(man, element, *self.get_arguments())
        calc_bounds(man, element, nn, nlb, nub, relu_groups, is_refine_layer=True)
        #nn.conv_counter += 1
        nn.ffn_counter += 1
        if testing:
