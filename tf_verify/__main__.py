        ...
        if(label == int(test[0])):
            ...
            if config.normalized_region==True:
                ...
                if(i==0):
                    ...
                    print(specLB)
                    print(specUB)
                    fullpath="input_box.csv"
                    '''
                    csvfile = open('new_img_1.csv', 'r')
                    lines = csv.reader(csvfile, delimiter=',')
                    line_in_list = list(lines)
                    specLB = np.float64(line_in_list[1])
                    specUB = np.float64(line_in_list[2])
                    print(specLB)
                normalize(specLB, means, stds, dataset)
                normalize(specUB, means, stds, dataset)
            else:
            ...
            if(i==0):
                ...
                with open(fullpath, 'a', newline='') as write_obj:
                    ...
                    csv_writer.writerow(specLB)
                    csv_writer.writerow(specUB)
            '''
